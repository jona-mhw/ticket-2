# ============================================
# TERRAFORM CONFIGURATION - MHW DEV
# ============================================
# Ambiente: Nube personal de desarrollo
...
# Obtener project number (necesario para IAP)
...
module "superuser_emails_secret" {
  ...
  project_id            = var.project_id
  secret_id             = "mhw-superuser-emails"
  service_account_email = var.service_account_email
}
...
module "cloud_run" {
  ...
  # Autoscaling (escala a cero para ahorrar costos)
  min_instances = "0"
  max_instances = "3"

  # Cloud SQL Proxy (no VPC Connector)
  ...
  env_vars = {
    ...
    FLASK_DEBUG       = "false"
    ENABLE_IAP        = "true"
    ENABLE_DEMO_LOGIN = "true" # MHW DEV tiene login tradicional habilitado
  }
  ...
  # Acceso público deshabilitado (solo IAP)
...
module "load_balancer" {
  ...
  # CDN deshabilitado (no necesario para dev)
  enable_cdn = false
  ...
  # Acceso IAP (usuarios/grupos autorizados)
