@echo off
REM ================================================
REM SCRIPT 2: DEPLOY CON RESET DB - QA
REM ================================================
...
gcloud run deploy ticket-home ^
  ...
  --max-instances=3 ^
  --concurrency=80 ^
  --set-env-vars="ENVIRONMENT=qa,FLASK_ENV=production,FLASK_DEBUG=false,ENABLE_IAP=true,ENABLE_DEMO_LOGIN=false,RESET_DB_ON_STARTUP=true,USE_QA_MINIMAL_SEED=true" ^
