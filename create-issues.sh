#!/bin/bash
# 🚀 Script para crear los 4 issues de mejora del proyecto
# Autor: Claude Code (tu agente favorito)
# Uso: ./create-issues.sh

set -e  # Salir si hay error

echo "🎯 Creando 4 issues épicos en GitHub..."
echo ""
...
gh issue create \
  --title "🧪 Implementar pytest con fixtures y coverage >80%" \
  --body "$(cat <<'EOF'
### 📋 Descripción
...
EOF
  ...
  --label "testing,enhancement,priority: high"
...
gh issue create \
  --title "🚀 Implementar CI/CD Pipeline con GitHub Actions" \
  --body "$(cat <<'EOF'
### 📋 Descripción
...
EOF
  ...
  --label "ci/cd,automation,priority: high"
...
gh issue create \
  --title "⚡ Implementar Redis Cache Layer para Performance" \
  --body "$(cat <<'EOF'
### 📋 Descripción
...
EOF
  ...
  --label "performance,enhancement,infrastructure"
...
gh issue create \
  --title "🔍 Optimizar Queries y Resolver N+1 Problems" \
  --body "$(cat <<'EOF'
### 📋 Descripción
...
EOF
  ...
  --label "performance,database,tech-debt"
...
echo "   https://github.com/jona-mhw/ticket-2/issues"
echo ""
