#!/bin/bash
# ===============================================
# SETUP SECRETS FOR v1.9.2 Security Hardening
# ===============================================
#
# Este script crea los secrets necesarios en Google Secret Manager
#
# EJECUTAR ESTE SCRIPT UNA SOLA VEZ ANTES DEL DEPLOYMENT
#
# ===============================================
...
# Emails de superusuarios
SUPERUSER_EMAILS="global_admin@tickethome.com;jonathan.segura@redsalud.cl;admin@tickethome.com"
...
if echo "$SUPERUSER_EMAILS" | gcloud secrets create superuser-emails \
    --data-file=- \
    --project=dev-ticket-home-redsalud 2>/dev/null; then
    echo "   ✅ Secret creado exitosamente"
else
    echo "   ⚠️  WARNING: Secret ya existe (OK si es re-ejecución)"
fi
echo ""
...
gcloud secrets add-iam-policy-binding superuser-emails \
    ...
    --project=dev-ticket-home-redsalud 2>/dev/null || true
...
if echo "$SUPERUSER_EMAILS" | gcloud secrets create superuser-emails \
    --data-file=- \
    --project=qa-ticket-home-redsalud 2>/dev/null; then
    echo "   ✅ Secret creado exitosamente"
else
    echo "   ⚠️  WARNING: Secret ya existe (OK si es re-ejecución)"
fi
...
gcloud secrets add-iam-policy-binding superuser-emails \
    --member="serviceAccount:qa-ticket-home@qa-ticket-home-redsalud.iam.gserviceaccount.com" \
    --role="roles/secretmanager.secretAccessor" \
    --project=qa-ticket-home-redsalud 2>/dev/null || true
echo "   ✅ OK - Permisos configurados"
echo ""
...
echo ""
echo "Próximos pasos:"
echo "   1. Ejecutar build de imagen Docker (1-build-push-DEV.sh)"
echo "   2. Ejecutar deployment a DEV (3-deploy-DEV-normal.sh)"
echo ""
